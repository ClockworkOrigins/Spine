...
namespace {
	...
	void ModInfoView::patchCheckG2() {
		...
		Database::DBError err;
		const std::vector<std::string> ids = Database::queryAll<std::string, std::string>(Config::BASEDIR.toStdString() + "/" + INSTALLED_DATABASE, "SELECT ModID FROM mods WHERE ModID = 40 OR ModID = 36 OR ModID = 116 OR ModID = 314;", err);
