...
namespace {
	...
	int jumpNr(const QByteArray & b, int & z, int & k) {
		int d2 = 0;
		int r = 0;
	...
	int readLength(const QByteArray & b, int & z) { // Liest die Länge des _variablesstrings
		int r = 0;
		z += 1;
...
SavegameManager::SavegameManager(QObject * par) : QObject(par) {
}

void SavegameManager::load(const QString & saveFile) {
	QFile f(saveFile);
...
void SavegameManager::save(const QString & saveFile, const QList<Variable> & variables) {
	QFile f(saveFile);
	if (!f.open(QIODevice::ReadOnly)) {
		return;
	}
